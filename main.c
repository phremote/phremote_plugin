...
)
{
	...
	case DLL_PROCESS_ATTACH:
	{
		...
		PluginInstance = PhRegisterPlugin(L"stage.WebStatus", Instance, &info);

		if (!PluginInstance)
			return FALSE;

		info->DisplayName = L"WebStatus";
		info->Author = L"stage";
		info->Description = L"RESTful service for process hacker";
		...
		// in DllMain. Settings must be added in DllMain.
		{
			static PH_SETTING_CREATE settings[] =
			{
				...
				{ IntegerSettingType, L"ProcessHacker.WebStatus.Port", L"1234" },
			};
...
)
{
	...
	myInteger = PhGetIntegerSetting(L"ProcessHacker.WebStatus.Port");
	StartRESTService();
...
{
	PhShowError((HWND)Parameter, L"Not implemented.");
}
...
)
{
	...
	{
		mcpu = (int)(processItem->CpuUsage * 100);
